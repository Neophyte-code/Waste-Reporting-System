...
<body class="font-[sans-serif] bg-gradient-to-r from-green-100 via-emerald-200 to-green-500 h-screen flex flex-col">
    <header class="bg-white w-full">
        <nav class="flex justify-between items-center w-[92%] mx-auto sm:py-0">
            ...
            <div class="nav-links duration-500 md:static absolute bg-white md:min-h-fit min-h-[38vh] left-0 top-[-100%] md:w-auto w-full flex items-center px-5 z-10">
                    ...
                    <div id="authModal" class="hidden absolute inset-0 flex items-center justify-center z-50 p-1 sm:p-2 md:p-4">
                        <div class="relative z-10 w-full flex justify-center items-center">
                            ...
                            <form id="signInForm" action="<?php echo URL_ROOT; ?>/auth/login" method="post" class="form-container form-visible flex flex-col gap-3 p-4 sm:p-4 md:p-6 lg:p-8  sm:gap-3 md:gap-4 bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-lg w-full max-w-xs sm:max-w-sm md:max-w-md  ">
                                ...
                                <?php endif; ?>
                                <input type="email" name="email" placeholder="Email" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors" aria-label="Email">
                                <div class="relative">
                                    <input type="password" name="password" placeholder="Password" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 pr-8 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors w-full" aria-label="Password">
                            ...
                            <form id="signUpForm" action="<?php echo URL_ROOT; ?>/auth/register" method="post" class="form-container form-hidden flex flex-col px-6 py-2 sm:px-8 sm:py-3 gap-2  lg:gap-4 bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-lg w-full max-w-xs sm:max-w-sm md:max-w-md ">
                                ...
                                <?php endif; ?>
                                <select name="barangay" id="barangay" class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors pr-8" aria-label="Barangay">
                                ...
                                <div class="flex gap-3 sm:gap-3">
                                    <input type="text" name="firstname" placeholder="First Name" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors w-full" aria-label="First Name">
                                    <input type="text" name="lastname" placeholder="Last Name" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors w-full" aria-label="Last Name">
                                </div>
                                <input type="email" name="email" placeholder="Email" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors" aria-label="Email">
                                <div class="relative">
                                    <input type="password" name="password" placeholder="Password" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 pr-8 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors w-full" aria-label="Password">
                                ...
                                <div class="relative">
                                    <input type="password" name="confirm_password" placeholder="Confirm Password" required class="text-green-900 border-2 border-green-300 rounded-lg p-1 sm:p-2 pr-8 font-medium text-xs sm:text-sm focus:border-green-500 transition-colors w-full" aria-label="Confirm Password">
                                    <button type="button" onclick="togglePassword(this)" class="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 text-green-600 hover:text-green-800 focus:outline-none transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                ...
                                <button type="submit" class="rounded-full bg-green-600 text-white hover:bg-green-700 p-1 sm:p-2 text-xs sm:text-sm font-semibold transition-colors">Register</button>
                                <p class="text-center text-green-800 text-xs sm:text-sm">Already have an account? <button type="button" id="showSignIn" class="text-green-600 hover:text-green-700 font-medium underline">Sign In</button></p>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Clear modal state from localStorage on page load/unload
            window.addEventListener('beforeunload', function() {
                localStorage.removeItem('modalState');
                localStorage.removeItem('activeForm');
            });

            // Also clear on page load in case the user comes back via browser navigation
            ...
            // Your existing code for restoring state (but it won't matter since we cleared it)
